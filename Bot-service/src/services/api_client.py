...
class ApiClient:
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"❌ Ошибка сервера при авторизации: {response.status} - {error_text[:200]}"
                    )
                    return {
                        "success": False,
            ...
            return {"success": False, "error": "Внутренняя ошибка", "details": str(e)}
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 201:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Ошибка сервера при регистрации: {response.status} - {error_text[:200]}"
                    )
                    return {
                        "success": False,
                        "error": f"Ошибка сервера: {response.status}",
    ...
    async def create_route_request(
        ...
        try:
            logger.info(
                f"🚀 Отправка запроса на создание маршрута для user_id: {user_id}"
            )
            ...
            async with self.session.post(
                ...
                if response.status == 202:
                    ...
                    logger.info(
                        f"✅ Запрос принят, task_id: {task_id}, user_id: {returned_user_id}"
                    )
                    return {
                        "success": True,
                        "task_id": task_id,
                    ...
                    logger.warning(
                        f"❌ Невалидный токен для пользователя {telegram_id}"
                    )
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Ошибка сервера при создании маршрута: {response.status} - {error_text[:200]}"
                    )
                    return {
                        "success": False,
                        "error": f"Ошибка сервера: {response.status}",
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Ошибка при получении пользователя: {response.status} - {error_text[:200]}"
                    )
                    return {
                        "success": False,
                        "error": f"Ошибка сервера: {response.status}",
            ...
            logger.error("❌ Ошибка подключения к API Gateway")
            return {"success": False, "error": "Не удалось подключиться к сервису"}
        except Exception as e:
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(
                        f"✅ Успешная авторизация по Telegram ID: {telegram_id}"
                    )
                    return {
                        "success": True,
                        "token": data.get("token"),
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Ошибка сервера при авторизации по Telegram: {response.status}"
                    )
                    return {
                        "success": False,
                        "error": f"Ошибка сервера: {response.status}",
