...
from src.config import (GIGACHAT_CREDENTIALS, GIGACHAT_MODEL, GIGACHAT_SCOPE,
...
class GigachatService:
    ...
    def __init__(self) -> None:
        if not GIGACHAT_CREDENTIALS:
            raise ValueError(
                "GIGACHAT_CREDENTIALS пуст. Укажи Authorization Key в env."
            )
    ...
    def _extract_json_object(text: str) -> Dict[str, Any]:
        ...
        if start != -1 and end != -1 and end > start:
            ...
            try:
                ...
                raise ValueError(
                    f"Не удалось распарсить JSON из ответа модели. Ошибка: {e}. Ответ: {text}"
                ) from e
    ...
    def _validate_input_points(points: List[Dict[str, str]]) -> None:
        ...
        for i, p in enumerate(points, 1):
            if "coordinates" not in p or "description" not in p:
                raise ValueError(
                    f"Элемент #{i} в points не содержит 'coordinates' или 'description': {p}"
                )
    ...
    def _validate_and_normalize_output(
        ...
        *,
        ...
        if missing:
            raise ValueError(
                f"В ответе отсутствуют обязательные поля {missing}. Ответ: {data!r}"
            )

        if not isinstance(data["output"], list):
            raise ValueError(
                "'output' должен быть списком, получено: " + repr(data["output"])
            )
        ...
        for i, item in enumerate(data["output"], 1):
            if not isinstance(item, dict):
                raise ValueError(
                    f"Элемент output #{i} не является объектом: {repr(item)}"
                )
            if "coordinates" not in item or "description" not in item:
                raise ValueError(
                    f"Элемент output #{i} не содержит 'coordinates' или 'description': {repr(item)}"
                )
    ...
    async def build_route(
        ...
        try:
            ...
            logger.warning("JSON parse failed, retrying with repair prompt: %s", e)
            repair_messages = [
