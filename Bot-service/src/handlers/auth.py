...
from utils.keyboards import (get_auth_keyboard, get_cancel_keyboard,
                             get_login_choice_keyboard, get_main_menu_keyboard)
...
async def process_register_username(message: types.Message, state: FSMContext):
    ...
    if len(username) < 3:
        await message.answer(
            "❌ Имя пользователя должно быть не менее 3 символов. Попробуйте снова:"
        )
        return
...
async def process_register_password(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "⏳ Регистрирую аккаунт...", reply_markup=ReplyKeyboardRemove()
    )
...
async def cancel_auth(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("❌ Действие отменено.", reply_markup=get_auth_keyboard())
...
async def link_telegram_account(
    token: str, user: types.User, email: str, password: str
) -> bool:
    ...
    try:
        async with api_client as client:
            ...
            response = await client.session.post(
                f"{client.base_url}/api/link_telegram", json=payload, headers=headers
            )
            ...
            logger.warning(
                "⚠️ Не удалось привязать Telegram ID: %s body=%s",
                response.status,
                body[:800],
            )
            return False
...
async def handle_webapp_data(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if str(telegram_id) != str(message.from_user.id):
            logger.warning(
                f"⚠️ Несоответствие Telegram ID: WebApp={telegram_id}, Message={message.from_user.id}"
            )
        ...
        await message.answer(
            success_text, reply_markup=get_main_menu_keyboard(is_authenticated=True)
        )
