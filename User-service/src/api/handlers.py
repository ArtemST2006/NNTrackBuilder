...
async def create_user(
    user_data: UserSignUpRequest, user_repo: UserRepository = Depends(get_user_repo)
):
...
@router.post(
    "/login_user", status_code=status.HTTP_200_OK, response_model=UserSignInResponse
)
async def login(
    user_data: UserSignInRequest, user_repo: UserRepository = Depends(get_user_repo)
):
    existing_user = await user_repo.get_by_email(user_data.email)
...
async def link_telegram(
    link_data: TelegramLinkRequest, user_repo: UserRepository = Depends(get_user_repo)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Ошибка при привязке аккаунта"
        )
...
async def get_user_by_telegram(
    telegram_id: str, user_repo: UserRepository = Depends(get_user_repo)
):
...
async def auth_by_telegram(
    auth_data: TelegramAuthRequest, user_repo: UserRepository = Depends(get_user_repo)
):
